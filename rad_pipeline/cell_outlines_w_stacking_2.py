...
def project_stacks(
    ...
    """
    # Channels that need z-projection — all others are intentionally skipped
    STACK_CHANNELS = (('ch2', 'dna'), ('ch5', 'agp'), ('ch8', 'mito'))
    ...
    for _, row in treatments.iterrows():
        ...
        for field in range(1, 10):
            ...
            # Collect planes for each channel we care about
            channel_planes = {
                ch_tag: _collect_planes(well_images, field_tag, ch_tag)
                for ch_tag, _ in STACK_CHANNELS
            }

            # Skip field entirely if none of the three channels have any planes
            if not any(channel_planes.values()):
                continue

            # Derive image_id (strip trailing p-component) from first plane found
            sample = next(
                p for planes in channel_planes.values() for p in planes
            )
            stem = Path(sample).stem.split('-')[0]
            image_id = re.sub(r'p\d+$', '', stem, flags=re.IGNORECASE)

            dest = output_dir / str(treatment)
            dest.mkdir(parents=True, exist_ok=True)

            for ch_tag, channel_name in STACK_CHANNELS:
                planes = channel_planes[ch_tag]
                if not planes:
                    continue
