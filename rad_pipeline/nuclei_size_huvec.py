...
#     dose_path = os.path.join(repo_path, week, "results", "huvec_control", "untreated")
...
def week_number(label):
    ...
    word = label.split("_")[1].lower()
    return words_to_nums.get(word, 0)

paired = sorted(zip(weeks, nuclei_size_avgs), key=lambda x: week_number(x[0]))
weeks_sorted, nuclei_sorted = zip(*paired)

# --- Select only week 2 ---
week2_index = next((i for i, w in enumerate(weeks_sorted) if "two" in w.lower()), None)
if week2_index is None:
    raise ValueError("Could not find 'week_two' in folder names!")

week2_data = nuclei_sorted[week2_index]

# --- Plot boxplot ---
fig, ax = plt.subplots(figsize=(10, 6))
bp = ax.boxplot(week2_data, patch_artist=True, widths=0.6)

colors = plt.cm.tab10.colors[:len(week2_data)]
for patch, color in zip(bp['boxes'], colors):
    patch.set_facecolor(color)
    patch.set_alpha(0.6)

ax.set_xticklabels(doses, rotation=45, ha="right")
ax.set_ylabel("Nuclei Size (AreaShape_Area)")
ax.set_title("Week 2 – Nuclei Size by Dose")
ax.set_yscale("log")
plt.tight_layout()

png_path = os.path.join(repo_path, "week2_nuclei_size_boxplot.png")
plt.savefig(png_path, dpi=300)
plt.close()
print(f"Saved boxplot for week 2 to {png_path}")
