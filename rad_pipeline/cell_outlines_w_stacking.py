...
def _collect_planes(well_images: list[str], field_tag: str, channel: str) -> list[str]:
    """Return all p-plane paths for a given field and channel, sorted by p-index."""
    planes: dict[int, str] = {}
    for img in well_images:
        if field_tag in img and channel in img:
            m = re.search(r'p(\d+)', img, re.IGNORECASE)
...
def project_stacks(
    ...
    """Max-project the p-planes for DNA (ch2), AGP (ch5), and Mito (ch8) for
    every r/c/f field, writing one projected TIFF per channel into
    output_dir / <treatment> / <image_id>_<channel>.tiff.

    RNA (ch4), ER (ch3), and brightfield (ch7) are single-plane channels and
    are copied as-is.
    """
    ...
    for _, row in treatments.iterrows():
        ...
        for field in range(1, 10):
            field_tag = f'f0{field}'
            if not any(field_tag in f for f in well_images):
                continue

            # Collect all p-planes for the three z-stacked channels
            dna_planes  = _collect_planes(well_images, field_tag, 'ch2')
            agp_planes  = _collect_planes(well_images, field_tag, 'ch5')
            mito_planes = _collect_planes(well_images, field_tag, 'ch8')

            if not any([dna_planes, agp_planes, mito_planes]):
                continue

            # Derive image_id from the first available plane filename
            sample_path = (dna_planes or agp_planes or mito_planes)[0]
            stem = Path(sample_path).stem.split('-')[0]
            image_id = re.sub(r'p\d+$', '', stem, flags=re.IGNORECASE)

            dest = output_dir / str(treatment)
            dest.mkdir(parents=True, exist_ok=True)

            # Project and write the three z-stacked channels
            for planes, channel_name in (
                (dna_planes,  'dna'),
                (agp_planes,  'agp'),
                (mito_planes, 'mito'),
            ):
            ...
            # Copy the single-plane channels as-is (first p-plane found)
            for ch_tag, channel_name in (('ch4', 'rna'), ('ch3', 'er'), ('ch7', 'brightfield')):
                matches = [f for f in well_images if field_tag in f and ch_tag in f]
                if matches:
                    shutil.copy(matches[0], dest / f'{image_id}_{channel_name}.tiff')
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help='The cell profiler pipeline to use',
        default=Path(
            '/lus/eagle/projects/FoundEpidem/astroka/mito_counting_v2.cppipe',
        ),
        type=Path,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='Output directory for max-projected z-stack TIFFs (DNA, AGP, Mito). '
             'If provided, projected images are written here before CellProfiler runs.',
        type=Path,
        default=None,
    )
    ...
    if args.projected_output_dir is not None:
        ...
        project_stacks(
            ...
            output_dir=args.projected_output_dir,
        )
